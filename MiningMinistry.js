    ...
    goHarvest: function(creepName, sourceNb) {
        ...
        if (myharvester.store.getFreeCapacity() > 0) {
            ...
            if (myharvester.harvest(sources[sourceNb]) == ERR_NOT_IN_RANGE) {
                if (myharvester.moveTo(sources[sourceNb], {visualizePathStyle: {stroke: '#ffffe0'}}) == "OK") {
                    myharvester.moveTo(sources[sourceNb], {visualizePathStyle: {stroke: '#ffffe0'}}).say('🔄 harvest');
                }
            ...
            if (myharvester.transfer(Game.spawns['Spawn1'], RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                if (myharvester.moveTo(Game.spawns["Spawn1"], {visualizePathStyle: {stroke: '#ffffe0'}}) == "OK") {
                    myharvester.moveTo(Game.spawns["Spawn1"], {visualizePathStyle: {stroke: '#ffffe0'}});
                }
                else console.log("Mining problem while coming back to spawn, the error code is "+myharvester.moveTo(Game.spawns["Spawn1"]));
            }
        }
    }
};
